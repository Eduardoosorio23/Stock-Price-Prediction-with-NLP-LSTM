...
#Reddit Posts    
    ...
    for i in range(num_months):#Number of months
...
#Tweets dataframe
    ...
    c.Since = str((currentDate - relativedelta(months=num_months)).date()) #return tweets published after this day(Months)
    c.Count = True
...
#webscraping stock price data on ticker symbol
    tick = web.DataReader(ticker, data_source='yahoo', start=str((currentDate - relativedelta(months=num_months)).date()), end=str(currentDate.date()))
